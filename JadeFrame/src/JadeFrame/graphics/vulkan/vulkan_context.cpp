...
static auto find_queue_families(VkPhysicalDevice physical_device, VkSurfaceKHR surface) -> QueueFamilyIndices {
	...
	for (u32 i = 0; i < queue_families.size(); i++) {
		...
		if (VK_SUCCESS != vkGetPhysicalDeviceSurfaceSupportKHR(physical_device, i, surface, &present_support)) {
			__debugbreak();
		}
...
Vulkan_Context::~Vulkan_Context() {
	...
	if (enable_validation_layers) {
		vkDestroyDebugUtilsMessengerEXT(m_instance.m_instance, m_instance.m_debug_messenger, nullptr);
	}
	vkDestroySurfaceKHR(m_instance.m_instance, m_surface, nullptr);
	vkDestroyInstance(m_instance.m_instance, nullptr);
}

auto Vulkan_Context::create_instance() -> void {

	m_instance.init();
}

auto Vulkan_Context::setup_debug_messenger() -> void {

}

auto Vulkan_Context::create_surface() -> void {
	std::cout << __FUNCTION__ << std::endl;
...
//}
	...
	VkResult result = vkCreateWin32SurfaceKHR(m_instance.m_instance, &create_info, nullptr, &m_surface);
...
auto Vulkan_Context::pick_physical_device() -> void {
	std::cout << __FUNCTION__ << std::endl;
	m_physical_devices = m_instance.query_physical_devices();
...
auto Vulkan_Context::create_swapchain() -> void {
	std::cout << __FUNCTION__ << std::endl;
	...
	VkExtent2D extent = choose_swap_extent(m_window_handle, swapchain_support.m_capabilities);
...
auto Vulkan_Context::create_image_views() -> void {
	std::cout << __FUNCTION__ << std::endl;
...
auto Vulkan_Context::create_render_pass() -> void {
	std::cout << __FUNCTION__ << std::endl;
...
auto Vulkan_Context::create_framebuffers() -> void {
	std::cout << __FUNCTION__ << std::endl;
...
auto Vulkan_Context::create_command_pool() -> void {
	std::cout << __FUNCTION__ << std::endl;
	QueueFamilyIndices queue_family_indices = find_queue_families(m_physical_device, m_surface);
...
auto Vulkan_Context::create_command_buffers() -> void {
	std::cout << __FUNCTION__ << std::endl;
...
auto Vulkan_Context::create_sync_objects() -> void {
	std::cout << __FUNCTION__ << std::endl;
...
auto VulkanInstance::query_physical_devices() -> std::vector<VkPhysicalDevice> {
	u32 device_count = 0;
