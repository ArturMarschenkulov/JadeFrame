...
static auto print_queue_families_info(std::vector<VulkanQueueFamily> queue_families) -> void {
    ...
    for (u32 i = 0; i < queue_families.size(); i++) {
        std::string str = "{ ";
        if (queue_families[i].m_properties.queueFlags & VK_QUEUE_GRAPHICS_BIT) { str += "Graphics "; }
        if (queue_families[i].m_properties.queueFlags & VK_QUEUE_COMPUTE_BIT) { str += "Compute "; }
        if (queue_families[i].m_properties.queueFlags & VK_QUEUE_TRANSFER_BIT) { str += "Transfer "; }
        if (queue_families[i].m_properties.queueFlags & VK_QUEUE_SPARSE_BINDING_BIT) { str += "SparseBinding "; }
        if (queue_families[i].m_properties.queueFlags & VK_QUEUE_PROTECTED_BIT) { str += "Protected "; }
        if (queue_families[i].m_present_support == VK_TRUE) { str += "Present "; }
        str += "}";
        Logger::info("Queue family {} has {} queues capable of {}", i, queue_families[i].m_properties.queueCount, str);
    }
}
