...
auto from_wstring_to_string(const std::wstring& wstr) -> std::string {
    ...
    i32 size_needed = WideCharToMultiByte(
        CP_UTF8, 0, &wstr[0], (i32)wstr.size(), NULL, 0, NULL, NULL);
    std::string str_to(size_needed, 0);
    WideCharToMultiByte(
        CP_UTF8, 0, &wstr[0], (i32)wstr.size(), &str_to[0], size_needed, NULL,
        NULL);
    return str_to;
}
auto from_string_to_wstring(const std::string& str) -> std::wstring {
    ...
    i32 size_needed =
        MultiByteToWideChar(CP_UTF8, 0, &str[0], (i32)str.size(), NULL, 0);
    std::wstring wstr_to(size_needed, 0);
    MultiByteToWideChar(
        CP_UTF8, 0, &str[0], (i32)str.size(), &wstr_to[0], size_needed);
    return wstr_to;
}
#endif
...
auto map_range(
    const f64 x, const f64 in_min, const f64 in_max, const f64 out_min,
    const f64 out_max) -> f64 {
