...
public:
    ...
    auto operator=(const DynamicArray& other) -> DynamicArray& {
        ...
        for (int i = 0; i < other.m_size; i++) { this->push_back(other.at(i)); }
    }
    ...
    auto at(const size_t& index) const -> T& { return m_array[index]; }

    auto push_back(const T& elem) -> void {
        const auto growth = m_capacity * 2;
        if (m_size == m_capacity) { this->reserve(growth); }
        m_array[m_size] = elem;
        m_size++;
    }

    auto push_back(std::initializer_list<T> list) -> void {
        for (const DynamicArray::T& elem : list) { this->push_back(elem); }
    }
...
auto dafoo() -> void { DynamicArray da = {1, 2, 3, 4}; }
